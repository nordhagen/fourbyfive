...
exports.processDir = async (options) => {
  ...
  const sharpening = SHARPENING_MAP[options.sharpen]
  const sharpening_sx = Object.keys(sharpening)
    .map((k) => k + '-' + sharpening[k])
    .join('_')
  console.log(sharpening_sx)
        ...
        if (stat.isFile() && IMAGE_FILE_EXP.test(file)) {
          try {
            const outputFile = path.join(
              ...
              file.replace(
                IMAGE_FILE_EXP,
                `_${options.aspect.join(
                  'x',
                )}_${sharpening_sx}_fourbyfive.jpg`,
              ),
            )
            ...
            sharp(filePath)
              ...
              .sharpen(
                !!options.sharpen
                  ? SHARPENING_MAP[options.sharpen]
                  : false,
              )
              .extend({
                top: padding,
            ...
            exit(
              `\nProcessing failed internally. Please report this issue at ${bugs.url}. Include the stack trace below and the options you used.\n\n${err.stack}`,
            )
          }
        }
      })
